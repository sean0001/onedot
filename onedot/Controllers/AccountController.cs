...
namespace one.OneDot.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new Auth_Users { UserName = model.Email, Email = model.Email };
                
                ///添加用户扩展信息，其他用户扩展信息也在此处 初始化。
                user.Auth_UserInfor = new Auth_UserInfor() { UserId = user.Id };
                ...
                if (result.Succeeded)
                {
                    ...
                    //SignInManager.UserManager.AddLoginAsync
                    //ExternalLoginInfo loginInfo = await AuthenticationManager.GetExternalLoginInfoAsync();

                    // 有关如何启用帐户确认和密码重置的详细信息，请访问 http://go.microsoft.com/fwlink/?LinkID=320771
                    // 发送包含此链接的电子邮件

                    if (Convert.ToString(System.Configuration.ConfigurationManager.AppSettings["CheckEmailOfUser"]) == "Enable") {
