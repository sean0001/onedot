...
namespace one.OneDot.Controllers
{
   // [Authorize]
    public class ManageController : Controller
    {
        ...
        public PartialViewResult StyleAndPassword()
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> ChangePassword3(ChangePasswordViewModel model)
        {

            if (!ModelState.IsValid)
            {
                return PartialView("pp/_ChangePassword",model);
            }
            ...
            if (result.Succeeded)
            {
                ...
                //return RedirectToAction("Index", new { Message = ManageMessageId.ChangePasswordSuccess });
                ViewBag.js = one.Core.Utilities.ShowSuccess;

                ModelState.AddModelError(Core.Enums.status.Success.ToString(), "密码修改成功！");
            }
...
#region 帮助程序
        ...
        private void AddErrors(IdentityResult result)
        {
            ...
            {
                ModelState.AddModelError(Core.Enums.status.Error.ToString(), error);
            }
        }
